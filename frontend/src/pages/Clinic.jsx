...
const Clinical = () => {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/departments?mainCategory=Clinical&isActive=true');
        if (res.ok) {
          const data = await res.json();
          setDepartments(data.departments || []);
        }
      } catch (error) {
        console.error("Failed to fetch clinical departments", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Direct patient care and advanced medical treatment facilities.
          </p>
        </div>
